...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // He posat la clau nova directament aquí per saltar-nos els problemes de Vercel
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'Ets el sommelier d\'en Pere. Respon en català de forma amable i experta. Aquests són els vins disponibles: ' + celler },
    ...
    if (data.choices && data.choices[0]) {
      ...
      res.status(200).json({ resposta: "Error de resposta de Groq: " + (data.error?.message || "Dades no vàlides") });
    }
